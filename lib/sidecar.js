...
/**
 * Returns the HTTP API port (health endpoint).
 * The sidecar serves HTTP on sidecarPort + 1 (default: 7843).
 */
function getHttpPort() {
    return config.sidecarPort + 1;
}
...
function isAlreadyRunning() {
    return new Promise((resolve) => {
        const req = http.get(
            `http://127.0.0.1:${getHttpPort()}/health`,
            { timeout: 2000 },
            (res) => {
...
async function spawn() {
    ...
    console.log(`[ENI] Spawning sidecar (ws: ${port}, http: ${port + 1})...`);

    try {
        sidecarProcess = cpSpawn(binaryPath, [], {
            stdio: 'pipe',
        });
