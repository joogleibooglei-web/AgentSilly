 ...
 *
 * Removes the existing target directory first to ensure stale files are cleaned,
 * then copies manifest.json and dist/ from the plugin root.
 * Skips the operation only if the installed version already matches.
...
function install() {
    ...
    try {
        // Remove existing target directory to ensure a clean install
        // This prevents stale files from previous versions lingering
