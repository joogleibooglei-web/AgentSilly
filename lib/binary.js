 ...
 * Falls back to reading the version from the latest git tag if the binary cannot
 * report its version (e.g., binary doesn't exist or doesn't support --version).
 *
 * @returns {string|null} Version string (e.g., "1.2.3") or null if version
 *   cannot be determined from any source.
 */
function getLocalVersion() {
    // Try getting version from the binary first
    try {
        ...
        // Binary doesn't exist or --version failed — fall through to git tag
    }

    // Fallback: derive version from the latest git tag in this repo
    try {
        ...
        // Not a git repo or no tags — give up
    }
...
/**
 * Always downloads the latest sidecar binary from GitHub releases.
 *
 * Skips version comparison — always pulls the latest release binary
 * to ensure the installed binary matches the latest published build.
 *
...
async function checkForUpdate() {
    ...
    if (!release) {
        console.error('[ENI] Could not fetch latest release — skipping update');
    ...
    console.log(`[ENI] Pulling latest sidecar binary v${remoteVersion} from GitHub releases...`);
    ...
    if (success) {
        console.log(`[ENI] Sidecar updated to v${remoteVersion}`);
